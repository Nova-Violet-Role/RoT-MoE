...
# --- 2a. the four invocation forms are the ONLY ones, and that is now asserted
#
# DEBT PAID.  wired_in() above recognises exactly four ways a workflow can run a
# checker:  "bash checker/X", "sh checker/X", "./checker/X", "source checker/X".
# Every one of today's wirings uses one of them -- MEASURED 2026-08-19: 93 textual
# occurrences of "checker/" across .github/workflows/*.yml, of which 88 sit behind
# one of the four forms and 5 are not commands at all (2 are 'paths:' filter
# entries in corpus-update.yml, 3 are prose inside echo strings).
#
# But that was a measurement, not an assertion, and the difference is the whole
# point of this file.  If someone later writes  exec checker/X.sh  or
# "$SHELL" checker/X.sh  or  env FOO=1 checker/X.sh , wired_in() cannot see it and
# the checker is reported NOT RUN BY ANY WORKFLOW while CI runs it every push.
# That direction fails LOUD rather than green, which is the safe direction -- but a
# gate that goes red for a wiring that genuinely exists is the gate people delete.
#
# So classify every mention instead of only recognising the good ones.  A mention
# is an invocation, a paths: entry, or a message.  Anything else stops the build
# and names the line, so the author either uses a standard form or extends
# wired_in() deliberately.  The residual, stated because it is real: a line that
# BOTH echoes and invokes through an unknown form reads as a message here.  The
# invocation arm is tested first, so any known form on such a line still wins.
mention_class () {   # <comment-stripped line> -> invocation | path-filter | message | UNCLASSIFIED
  case "$1" in
    *"bash checker/"*|*"sh checker/"*|*"./checker/"*|*"source checker/"*)
      printf '%s' "invocation" ;;
    *"- 'checker/"*|*'- "checker/'*|*"- checker/"*)
      printf '%s' "path-filter" ;;
    *echo*|*printf*)
      printf '%s' "message" ;;
    *)
...
# CONTROL: all four classes reachable, and an unknown invocation form really is
# refused.  Re-trip this whenever mention_class changes.
mc_bad=0
[ "$(mention_class '      - run: bash checker/dorks.sh --order')" = "invocation" ]  || mc_bad=1
[ "$(mention_class "      - 'checker/repo-complete.sh'")" = "path-filter" ]         || mc_bad=1
[ "$(mention_class '        echo "generated by checker/status-verdict.sh"')" = "message" ] || mc_bad=1
[ "$(mention_class '      - run: exec checker/ghost.sh')" = "UNCLASSIFIED" ]        || mc_bad=1
if [ "$mc_bad" -eq 0 ]; then
  ok "CONTROL: mention_class separates invocation, paths: entry, message and an unknown form"
else
  bad "CONTROL FAILED: mention_class no longer separates the four mention classes"
fi

mentions=0
unclassified=0
for wfy in "$REPO"/.github/workflows/*.yml; do
  [ -f "$wfy" ] || continue
  wfn="${wfy##*/}"
  lineno=0
  while IFS= read -r rawline; do
    ...
    mentions=$((mentions + 1))
    if [ "$(mention_class "$line")" = "UNCLASSIFIED" ]; then
      unclassified=$((unclassified + 1))
      bad "UNCLASSIFIED MENTION of checker/ at $wfn:$lineno"
      echo "        $(printf '%s' "$line" | sed 's/^ *//' | cut -c1-90)"
      echo "        Not one of the four invocation forms wired_in() knows, not a"
      echo "        paths: entry, not a message.  If this RUNS a checker, the wiring"
      echo "        test above is blind to it and that checker will be reported as"
      echo "        never run by any workflow.  Use a standard form, or extend"
      echo "        wired_in() AND mention_class() together and re-trip the controls."
    fi
  done < "$wfy"
done
if [ "$unclassified" -eq 0 ]; then
  ok "all $mentions mentions of checker/ in workflows are invocations, paths: entries or messages"
fi
