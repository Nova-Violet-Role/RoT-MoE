...
#
# The version number IS the variant. This is not a roadmap where 0.2.2 succeeds
# 0.2.1; all three are built from one tree, in one run, and shipped together:
#
#   0.2.0  core       the router. No Lean, no fetcher, NO NETWORK AT ALL.
#   0.2.1  lean       core + the Lean 4 toolchain fetcher + the proof corpus.
#   0.2.2  unsealed   lean + the axiom classifier + the policy that permits
...
# downloads 0.2.0 and finds a manifest saying 0.2.2 has been handed a file that
...
VARIANTS="core:0.2.0 lean:0.2.1 unsealed:0.2.2"
...
# in checker/ and therefore already ships with LEAN; what 0.2.2 adds is the
...
if [ -s "$L_CORE" ]; then
  ...
  [ "$leak" -eq 0 ] && ok "CORE (0.2.0) carries no fetcher, no corpus, no unsealed page -- it cannot download anything"
fi
...
if [ -s "$L_LEAN" ]; then
  ...
  has "$L_LEAN" '^UNSEALED.md$' && { bad "LEAN contains UNSEALED.md -- 0.2.1 and 0.2.2 would be the same artifact"; short=$((short+1)); }
  [ "$short" -eq 0 ] && ok "LEAN (0.2.1) carries both fetchers, the pinned toolchain, all $nmod proof module(s), and NOT the unsealed page"
fi
...
if [ -s "$L_UNS" ]; then
  ...
  [ "$u" -eq 0 ] && ok "UNSEALED (0.2.2) carries the unsealed page AND the axiom classifier"
fi
