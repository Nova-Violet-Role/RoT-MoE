...
[![Pure Router](https://img.shields.io/badge/v0.2.0-Pure%20Router-0969da?style=flat-square)](#-v010--pure-router)
[![Router + Lean](https://img.shields.io/badge/v0.2.1-Router%20%2B%20Lean%204-1a7f37?style=flat-square)](#-v011--router--lean-4)
[![Unsealed](https://img.shields.io/badge/v0.2.2-Router%20%2B%20Lean%20%2B%20Extra-8250df?style=flat-square)](#-v012--router--lean--extra)
[![Zero sorry](https://img.shields.io/badge/sorry-0-27ae60?style=flat-square)](#)
[![Theorems](https://img.shields.io/badge/theorems-144-2C3E50?style=flat-square)](#)
...
|---|---|---|
| **`v0.2.0`** | **Pure Router** | the plugin, alone. No Lean, no fetcher, **no network at all** |
| **`v0.2.1`** | **Router + Lean 4** | the above **plus the full Lean 4 toolchain** — the workshop these proofs were built in |
| **`v0.2.2`** | **Router + Lean + Extra** | the above **plus `native_decide` unsealed**, and the instrument that keeps that honest |
...
**Take `v0.2.0` — Pure Router.** It is the plugin. It routes, it gauges, it
...
**Take `v0.2.1` — Router + Lean 4** if you want **the machine that makes the
theorems**, not just the theorems.

Re-checking our 144 proofs is the *smallest* thing it does — it is the demo, not
...
**Take `v0.2.2` — Router + Lean + Extra** if you want the discipline in *your*
...
> hooks. `v0.2.0` is the product. **`v0.2.1` is the product plus the workshop it
> was built in. `v0.2.2` hands you the keys to the workshop's locked drawer, and
...
## 🔬 `v0.2.1` — Router + Lean 4

**`rot-moe-0.2.1-lean.zip`**
...
| ✅ **Pro** | **You can falsify us.** 144 theorems, 12 modules, `lake build` → `#print axioms` → `leanchecker`, on your hardware — the demo, not the point |
...
## ⚗️ `v0.2.2` — Router + Lean + Extra

**`rot-moe-0.2.2-unsealed.zip`**
...
So `native_decide` is **already available in `v0.2.1`**. It is not withheld by
...
| ⚠️ **Con** | The 144 theorems in this repository **stay `native_decide`-free**. Unsealing applies to your work, not ours — the headline count would otherwise stop meaning what it says |
...
Measured on this corpus by the tool itself: **144 KERNEL, 0 COMPILER, 0 BROKEN.**
...
* **No system directory.** Nothing outside the plugin folder and — for `v0.2.1`,
...
unzip rot-moe-0.2.0-core.zip -d rot-moe
...
**`v0.2.1` only — the optional toolchain, after you have read what it does:**
