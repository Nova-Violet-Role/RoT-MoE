...
[![Pure Router](https://img.shields.io/badge/v0.6.0-Pure%20Router-0969da?style=flat-square)](#-v010--pure-router)
[![Router + Lean](https://img.shields.io/badge/v0.6.1-Router%20%2B%20Lean%204-1a7f37?style=flat-square)](#-v011--router--lean-4)
[![Unsealed](https://img.shields.io/badge/v0.6.2-Router%20%2B%20Lean%20%2B%20Extra-8250df?style=flat-square)](#-v012--router--lean--extra)
...
|---|---|---|
| **`v0.6.0`** | **Pure Router** | the plugin, alone. No Lean, no fetcher, **no network at all** |
| **`v0.6.1`** | **Router + Lean 4** | the above **plus the full Lean 4 toolchain** — the workshop these proofs were built in |
| **`v0.6.2`** | **Router + Lean + Extra** | the above **plus `native_decide` unsealed**, and the instrument that keeps that honest |
...
**Take `v0.6.0` — Pure Router.** It is the plugin. It routes, it gauges, it
...
**Take `v0.6.1` — Router + Lean 4** if you want **the machine that makes the
...
**Take `v0.6.2` — Router + Lean + Extra** if you want the discipline in *your*
...
> hooks. `v0.6.0` is the product. **`v0.6.1` is the product plus the workshop it
> was built in. `v0.6.2` hands you the keys to the workshop's locked drawer, and
...
## 🔬 `v0.6.1` — Router + Lean 4

**`rot-moe-0.6.1-lean.zip`**
...
## ⚗️ `v0.6.2` — Router + Lean + Extra

**`rot-moe-0.6.2-unsealed.zip`**
...
So `native_decide` is **already available in `v0.6.1`**. It is not withheld by
...
| ✅ **Pro** | Everything in `v0.6.1` — the toolchain, the corpus, the mutation suites |
...
| ⚠️ **Con** | Nothing new is downloaded. `v0.6.1` already has the binaries; `v0.6.2` changes what the gates *permit* and hands you the instrument — it does not add a compiler |

**Contains:** everything in `v0.6.1` **plus** `UNSEALED.md` — the page that names
...
* **No system directory.** Nothing outside the plugin folder and — for `v0.6.1`,
...
unzip rot-moe-0.6.0-core.zip -d rot-moe
...
**`v0.6.1` only — the optional toolchain, after you have read what it does:**
