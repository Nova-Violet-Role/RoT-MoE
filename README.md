...
them, and **1648 machine-checked theorems in Lean 4** state what that gauge must
...
|---|---|---|
| Lean modules in `lean/Proofs/` | **89** | `ls lean/Proofs/*.lean \| wc -l` |
| theorems and lemmas proved | **1648** | `bash checker/count-theorems.sh lean/Proofs/*.lean` |
| mutation suites | **77** | `ls lean/mutate/mutate_*.sh \| wc -l` |
| checkers | **81** | `ls checker/*.sh \| wc -l` |
...
| `RoT-MoE-Router-Lean.zip` | adds `lean/` — the proof corpus and its mutation suites — plus `checker/` (81 checkers) and `SETUP_LEAN`, for re-proving every claim on your own machine |
...
| The corpus is real | **MEASURED** | **89 modules**, **1648 theorems**, 77 mutation suites, **797 mutants applied, 797 killed, 0 survived, 0 discarded** |
