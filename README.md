...
them, and **1662 machine-checked theorems in Lean 4** state what that gauge must
...
|---|---|---|
| Lean modules in `lean/Proofs/` | **90** | `ls lean/Proofs/*.lean \| wc -l` |
| theorems and lemmas proved | **1662** | `bash checker/count-theorems.sh lean/Proofs/*.lean` |
| mutation suites | **77** | `ls lean/mutate/mutate_*.sh \| wc -l` |
| checkers | **82** | `ls checker/*.sh \| wc -l` |
...
| `RoT-MoE-Router-Lean.zip` | adds `lean/` — the proof corpus and its mutation suites — plus `checker/` (82 checkers) and `SETUP_LEAN`, for re-proving every claim on your own machine |
...
| The corpus is real | **MEASURED** | **90 modules**, **1662 theorems**, 77 mutation suites, **797 mutants applied, 797 killed, 0 survived, 0 discarded** |
