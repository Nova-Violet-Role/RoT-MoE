...
**This file ships only in `v0.5.2`.** It is the difference between that variant
and `v0.5.1`, written down, because a tier whose extra content cannot be named
is not a tier.
...
`v0.5.1` gives you the toolchain with our discipline enforced. `v0.5.2` gives
...
> **`native_decide` is already possible in `v0.5.1`.** Nothing was withheld.
...
> and `v0.5.2` changes the policy, not the payload.
