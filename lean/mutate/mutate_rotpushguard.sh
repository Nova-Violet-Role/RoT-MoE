...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotPushGuard.lean (a partial run is not a pass)
#
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotPushGuard.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotpushguard.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotPushGuard ) >/tmp/mut_pre_rotpushguard.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotPushGuard)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotpushguard.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotPushGuard ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotPushGuard ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotPushGuard mutation suite ==="
...
run_mut P01 "def mayPush (s : PromiseState) (_t : PushTarget) : Bool := isFulfilled s" "def mayPush (s : PromiseState) (t : PushTarget) : Bool := match t with | .mainBranch => isFulfilled s | _ => true" \
  "the_target_cannot_change_the_verdict -- this IS the incident: let the destination into the decision and a side branch becomes freely pushable while main stays guarded, which is precisely the reasoning that was used"
run_mut P02 "  allObligations.all (fun o => s.met.contains o)" "  allObligations.any (fun o => s.met.contains o)" \
  "permission_is_exactly_an_empty_outstanding_list -- all becomes any and ONE met obligation out of six unlocks the remote"
run_mut P03 "   .verifyRunOnMain]" "   ]" \
  "the_empty_state_is_refused_on_every_target -- drop an obligation from the ledger and the promise can be declared fulfilled without it; the guard shrinks silently"
run_mut P04 "  [.corpus40, .pilot12Pairs, .sessions160, .preferenceMeasured, .p22Established," "  [.pilot12Pairs, .pilot12Pairs, .sessions160, .preferenceMeasured, .p22Established," \n  "the_empty_state_is_refused_on_every_target -- duplicate one obligation over another and the ledger still has six entries while silently checking corpus40 never; a count-based reader would call this healthy"
