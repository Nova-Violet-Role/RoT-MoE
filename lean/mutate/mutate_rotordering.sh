...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotOrdering.lean (why one ordering cannot attribute)
#
...
#
# WHAT THIS SUITE IS AIMED AT. The module turns bench/trap-latency.js's REFUSAL
# to attribute a speedup from one ordering into a theorem. Three groups:
#
#   O01-O04  THE COST MODEL. Break how a duration is composed. If the additive
#            structure can be damaged without a theorem noticing, the
#            identifiability results are about nothing.
#   O05-O07  THE NON-IDENTIFIABILITY WITNESSES. Break the pair of models that
#            proves one ordering is ambiguous. A surviving mutant here means
#            the ambiguity was asserted, not exhibited.
#   O08-O11  THE CONTROL'S DECISIVENESS. Break the two worlds whose predictions
#            DIVERGE b-first. These are what make running the control worth the
#            wall time rather than a gesture.
#
# =============================================================================
...
echo "=== RotOrdering mutation suite ==="
...
run_mut O01 "| .unrouted => 0 | .routed => m.armEffect" "| .unrouted => 0 | .routed => 0" "the routed arm pays armEffect in its duration"
run_mut O02 "| .first => 0 | .second => m.posEffect" "| .first => 0 | .second => 0" "a turn in second position pays posEffect"
run_mut O03 "duration m .routed .second - duration m .unrouted .first" "duration m .routed .first - duration m .unrouted .first" "the a-first gap puts the ROUTED arm second"
run_mut O04 "duration m .routed .first - duration m .unrouted .second" "duration m .routed .second - duration m .unrouted .second" "the b-first gap puts the ROUTED arm first"
run_mut O05 "gapAFirst m = m.armEffect + m.posEffect" "gapAFirst m = m.armEffect - m.posEffect" "the a-first gap is the SUM of the two effects"
run_mut O06 "gapBFirst m = m.armEffect - m.posEffect" "gapBFirst m = m.armEffect + m.posEffect" "the b-first gap is their DIFFERENCE"
run_mut O07 "refine ⟨⟨0, -100, 0⟩, ⟨0, 0, -100⟩, ?_, ?_, ?_⟩ <;> decide" "refine ⟨⟨0, -100, 0⟩, ⟨0, -100, 0⟩, ?_, ?_, ?_⟩ <;> decide" "the two ambiguous witnesses really do disagree about the cause"
run_mut O08 "def pureCacheWorld : Model := ⟨1000, 0, -400⟩" "def pureCacheWorld : Model := ⟨1000, -400, 0⟩" "the pure-cache world attributes nothing to the router"
run_mut O09 "def pureRouterWorld : Model := ⟨1000, -400, 0⟩" "def pureRouterWorld : Model := ⟨1000, 0, -400⟩" "the pure-router world attributes nothing to position"
run_mut O10 "def mixedWorld : Model := ⟨1000, -300, -100⟩" "def mixedWorld : Model := ⟨1000, -400, 0⟩" "the mixed world splits the effect 300/100"
run_mut O11 "gapAFirst m + gapBFirst m = 2 * m.armEffect" "gapAFirst m + gapBFirst m = 2 * m.posEffect" "recovering armEffect uses the SUM of both gaps"
