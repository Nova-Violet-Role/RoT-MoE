...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotDebugLog.lean (a null can belong to the analysis, not the world)
#
...
#
# WHAT THIS SUITE IS AIMED AT. RotDebugLog states why the round-1 A/B verdict
...
F="Proofs/RotDebugLog.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotDebugLog.olean
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotDebugLog ) >/tmp/mut_pre_rotsymbiogenesis.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotDebugLog)."
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotDebugLog ) >/dev/null 2>&1' EXIT
...
# 2026-08-07, being killed at that ceiling left a MUTATED RotDebugLog.lean on
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotDebugLog ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotDebugLog\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotDebugLog mutation suite ==="
...
# --- mutants ------------------------------------------------------------------
# Every theorem in RotDebugLog is proved by decide or a two-line simp over
...
run_mut D01 \
  ...
  'def recordsSeen (w : World) : Nat := w.fired' \
  'silent_channel_is_ambiguous'

# The marker deleted: back to the silent channel the repair replaced.
run_mut D02 \
  '    marker := !w.writable && 0 < w.fired }' \
  '    marker := false }' \
  'marker_resolves_the_ambiguity'

# The lazy fix -- warn every turn. It distinguishes nothing, and this is the
# theorem that forbids it.
run_mut D03 \
  '    marker := !w.writable && 0 < w.fired }' \
  '    marker := true }' \
  'marker_is_not_always_set'

# Truncate from the FRONT: bounded, and it throws away the newest turn, which
# is the only one anybody debugging is looking for.
run_mut D04 \
  '  records.drop (records.length - cap)' \
  '  records.take cap' \
  'rotate_keeps_the_newest'

# Drops from the front by a fixed amount, so a log under the cap loses records
# it had no need to lose.
run_mut D05 \
  '  records.drop (records.length - cap)' \
  '  records.drop cap' \
  'rotate_below_cap_is_identity'

# A contract that everything satisfies. This is what a decorative check looks
# like from the inside.
run_mut D06 \
  '  c.tolerantWrite && c.marksLostAppend && c.boundedKeepingNewest' \
  '  true' \
  'shipped_hook_failed_the_contract'

# Claims the pre-repair hook already satisfied the contract -- the exact
# overclaim the finding exists to refute.
run_mut D07 \
  '  { tolerantWrite := true, marksLostAppend := false, boundedKeepingNewest := false }' \
  '  { tolerantWrite := true, marksLostAppend := true, boundedKeepingNewest := true }' \
  'shipped_hook_failed_the_contract'

# An observation that always reads zero cannot support the inference that
# zero-and-unmarked means the router was quiet.
run_mut D08 \
  '  { records := recordsSeen w,' \
  '  { records := 0,' \
  'quiet_and_unmarked_means_it_never_fired'

# `|| true` alone declared sufficient -- the answer this module was written to
# reject.
run_mut D09 \
  '  c.tolerantWrite && c.marksLostAppend && c.boundedKeepingNewest' \
  '  c.tolerantWrite' \
  'tolerance_alone_is_insufficient'
...
if [ "$filtered" -eq 1 ]; then
  echo "=== RotDebugLog: PARTIAL RUN (MUT_ONLY='$MUT_ONLY') -- $killed killed, $survived survived, $discarded discarded, $skipped SKIPPED ==="
  echo "NOT a suite result. $skipped mutants were never applied and prove nothing."
  [ "$survived" -eq 0 ] && [ "$discarded" -eq 0 ] && exit 3
  exit 1
fi
echo "=== RotDebugLog: $killed killed, $survived survived, $discarded discarded ==="
