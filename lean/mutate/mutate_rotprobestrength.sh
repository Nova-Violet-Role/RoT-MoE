...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotProbeStrength.lean (a partial run is not a pass)
#
...
F="Proofs/RotProbeStrength.lean"
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotProbeStrength.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotprobestrength.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotProbeStrength ) >/tmp/mut_pre_rotprobestrength.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotProbeStrength)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotprobestrength.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotProbeStrength ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotProbeStrength ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotProbeStrength mutation suite ==="
...
run_mut S01 "  | .nonEmpty, k => 1 ≤ k" "  | .nonEmpty, k => 0 ≤ k" \
  "nonEmpty_is_sound_for_a_single_artifact -- a non-emptiness test that accepts ZERO evidence witnesses nothing at all; this is the degenerate end of the same defect, where even the three rows that are legitimately served by test -s would stop being served"
run_mut S02 "  | .atLeast n, k => n ≤ k" "  | .atLeast n, k => k ≤ n" \
  "atLeast_is_sound, atLeast_is_complete -- reverse the comparison and the counting probe accepts exactly the states it should refuse, which is how a wc -l guard written backwards passes review"
run_mut S03 "  | .nonEmpty => o.required ≤ 1" "  | .nonEmpty => true" \
  "the_shipped_ledger_was_not_sound, probeSound_implies_sound -- THE mutant for this module: declare every non-emptiness probe adequate and the shipped ledger passes its own check, which is precisely the state push-guard.sh was in before this commit"
run_mut S04 "  [({ name := \"corpus40\",           required := 40  }, .atLeast 40)," "  [({ name := \"corpus40\",           required := 40  }, .nonEmpty)," \
  "the_repaired_ledger_is_sound, every_repaired_row_is_semantically_sound -- re-introduce the exact shipped defect into the repaired ledger; if this survives, the repair was never checked"
run_mut S05 "  ∀ k, accepts p k = true → o.required ≤ k" "  ∃ k, accepts p k = true → o.required ≤ k" \
  "nonEmpty_cannot_witness_a_counted_obligation, atLeast_is_sound -- soundness quantified existentially is satisfied by ANY probe, so the central indictment would evaporate while every theorem name stayed the same"
run_mut S06 "def ledgerOk (rows : List Row) : Bool := rows.all rowSound" "def ledgerOk (rows : List Row) : Bool := rows.any rowSound" \
  "the_shipped_ledger_was_not_sound -- with any, one correct row (pilot12Pairs counted to 12) vouches for five weak ones; a ledger check that passes on a single good row is the aggregation defect this repo has already found twice"
run_mut S07 "  [({ name := \"corpus40\",           required := 40  }, .nonEmpty)," "  [({ name := \"corpus40\",           required := 1   }, .nonEmpty)," \
  "the_shipped_ledger_was_not_sound, the_repair_kept_every_obligation -- lowering the DEMAND instead of strengthening the probe is the tempting fake repair; it makes the check pass without any more evidence existing"
run_mut S08 "    ¬ complete (Probe.atLeast (o.required + 1)) o := by" "    ¬ complete (Probe.atLeast o.required) o := by" \
  "an_inflated_probe_refuses_a_finished_obligation -- the theorem that keeps the repair from overshooting into a wall; if it can be restated about the CORRECT probe and still build, it was saying nothing about inflation"
