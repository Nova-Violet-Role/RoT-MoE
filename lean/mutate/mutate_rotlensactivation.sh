...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotLensActivation.lean (a partial run is not a pass)
#
...
F="Proofs/RotLensActivation.lean"
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotLensActivation.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotlensactivation.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotLensActivation ) >/tmp/mut_pre_rotlensactivation.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotLensActivation)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotlensactivation.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLensActivation ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLensActivation ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotLensActivation mutation suite ==="
...
run_mut L01 "  if mentions names lens then 1 else 0" "  if mentions names lens then 2 else 0" \
  "the_router_can_never_report_more_than_one_active_lens, fusion_is_unreachable -- the whole finding is that _br is ASSIGNED 1 and therefore bounded; a 2 here is exactly the FUSE the spec describes and the shipped router does not implement"
run_mut L02 "  | b :: bs => (if b then 1 else 0) + trueBreadth bs" "  | b :: bs => trueBreadth bs" \
  "the_vector_counts_what_the_names_hit -- a trueBreadth that counts nothing makes the assignment trivially honest, which would let the duplicate-roster defect through unnoticed"
run_mut L03 "  | n :: ns => !mentions ns n && distinct ns" "  | n :: ns => distinct ns" \
  "distinct_names_hold_the_count_to_one, the_shipped_names_are_distinct -- a distinctness test that ignores duplicates is satisfied by every roster, so the hypothesis protecting the breadth assignment would be vacuous"
run_mut L04 "def term (l : Lens) (a : Bool) : Nat := l.lam * l.sig * l.mu * (1 + hOne a)" "def term (l : Lens) (a : Bool) : Nat := l.lam * l.sig * l.mu * (1 + hOne true)" \
  "the_active_lens_counts_exactly_double -- freeze the entropy factor and activation stops costing anything, which is the (1+H) term the live gauge actually applies"
run_mut L05 "def gauge (ls : List (Lens × Bool)) : Nat := (ls.map (fun p => term p.1 p.2)).sum" "def gauge (ls : List (Lens × Bool)) : Nat := ((ls.filter (fun p => p.2)).map (fun p => term p.1 p.2)).sum" \
  "raising_an_inactive_lens_raises_the_gauge, silencing_the_eight_changes_the_answer -- THE mutant for this module: score only the routed lens and 'nine-lens' becomes decoration, because the other eight would no longer touch R/s+"
run_mut L06 "   (⟨12, 1743, 110⟩, false),   -- Eidolon" "   -- Eidolon removed" \
  "forgeClaudeTurn.length = 9 -- drop a lens from the witness and the 26.9 percent contributed by the silent eight is measured over an ensemble that is not the shipped one"
run_mut L07 "   \"Eidolon\", \"Claude\"]" "   \"Eidolon\", \"Eidolon\"]" \
  "the_shipped_names_are_distinct, both_observed_breadths_are_reachable -- a duplicated roster entry is precisely the case where the assigned breadth undercounts the vector it accompanies"
