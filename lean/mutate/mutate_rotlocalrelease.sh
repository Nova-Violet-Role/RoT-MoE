...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotLocalRelease.lean (a null can belong to the analysis, not the world)
#
...
#
# WHAT THIS SUITE IS AIMED AT. RotLocalRelease states why the round-1 A/B verdict
...
F="Proofs/RotLocalRelease.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotLocalRelease.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutlocal.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotLocalRelease ) >/tmp/mut_pre_rotlocalrelease.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotLocalRelease)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotlocalrelease.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLocalRelease ) >/dev/null 2>&1' EXIT
...
# 2026-08-07, being killed at that ceiling left a MUTATED RotLocalRelease.lean on
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLocalRelease ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotLocalRelease\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotLocalRelease mutation suite ==="
...
# --- mutants ------------------------------------------------------------------
# Every theorem in RotLocalRelease is proved by decide or a two-line simp over
...
run_mut L01 \
  ...
  'def publishable (_v : Version) : Bool := true' \
  'local_never_publishable'

# Freshness stops caring WHICH commit built the artifact -- the stale case,
# reinstalled as the definition. An old zip whose contents happen to match is
# read as evidence about today's head.
run_mut L02 \
  '  a.builtFrom = head && a.digest = regenDigest' \
  '  a.digest = regenDigest' \
  'stale_is_not_fresh'

# The contents stop being compared at all: same commit is enough. This is the
# 'the file is there, so it is fine' failure the module was written against.
run_mut L03 \
  '  a.builtFrom = head && a.digest = regenDigest' \
  '  a.builtFrom = head' \
  'fresh_survives_rebuild'

# A digest tool that produced NO answer is scored as a match. This is the
# &&-chain defect in its most dangerous direction -- a broken instrument
# reporting a pass.
run_mut L04 \
  '  | none => Compared.unmeasured' \
  '  | none => Compared.same' \
  'unmeasured_is_not_evidence'

# The other direction: a broken instrument reported as a broken artifact.
# Less dangerous, still wrong -- it sends the reader to debug a zip that was
# never compared.
run_mut L05 \
  '  | none => Compared.unmeasured' \
  '  | none => Compared.differs' \
  'unmeasured_is_not_differs'

# Nothing is ever evidence. A checker built on this can never pass, which is
# the failure mode that gets a gate deleted rather than fixed.
run_mut L06 \
  '  | Compared.same => true' \
  '  | Compared.same => false' \
  'evidence_implies_fresh'

printf '


restoring baseline artifact ... '
if ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLocalRelease ) >"$LOG/restore.log" 2>&1; then
...
if [ "$filtered" -eq 1 ]; then
  echo "=== RotLocalRelease: PARTIAL RUN (MUT_ONLY='$MUT_ONLY') -- $killed killed, $survived survived, $discarded discarded, $skipped SKIPPED ==="
  echo "NOT a suite result. $skipped mutants were never applied and prove nothing."
  [ "$survived" -eq 0 ] && [ "$discarded" -eq 0 ] && exit 3
  exit 1
fi
echo "=== RotLocalRelease: $killed killed, $survived survived, $discarded discarded ==="
