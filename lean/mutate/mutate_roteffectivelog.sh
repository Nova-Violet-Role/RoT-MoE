...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEffectiveLog ) >/dev/null 2>&1' EXIT
...
# Measured 2026-08-09 on Proofs.RotEffectiveLog.
cp "$BAK" "$F" 2>/dev/null
( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEffectiveLog ) >/dev/null 2>&1
_base=$?
if [ "$_base" -ne 0 ]; then
