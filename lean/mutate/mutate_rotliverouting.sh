...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotLiveRouting.lean (live lane coverage and the gauge)
#
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotLiveRouting.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutliverouting.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotLiveRouting ) >/tmp/mut_pre_rotliverouting.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotLiveRouting)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotliverouting.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLiveRouting ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLiveRouting ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
run_mut L01 'def lanesReached : Nat := 10' 'def lanesReached : Nat := 1' 'the live router is made lane-constant -- the_live_router_is_not_lane_constant and every_declared_lane_fired must die'
run_mut L02 'def lanesDeclared : Nat := 10' 'def lanesDeclared : Nat := 11' 'a declared lane is added with no measurement behind it, so full coverage is no longer true'
run_mut L03 'def gaugeValues : Nat := 10' 'def gaugeValues : Nat := 1' 'the gauge is made a constant and the dynamism claim must fail'
run_mut L04 'def liveSessions : Nat × Nat := (4, 4)' 'def liveSessions : Nat × Nat := (4, 3)' 'one live session misses its lane and every_live_session_hit_its_lane must die'
run_mut L05 'def liveDistinctLanes : Nat := 4' 'def liveDistinctLanes : Nat := 1' 'the corrected probe is made as blind as the old one -- both variation theorems must fail'
run_mut L06 'def preFixDistinctLanes : Nat := 1' 'def preFixDistinctLanes : Nat := 4' 'the defect is rewritten as if it had passed, erasing the repair this module records'
run_mut L07 '  if sessionLevel then 0 else chars' '  if sessionLevel then chars else chars' 'a session-level route is given the prompt it does not have, so the ignores-the-prompt proof breaks'
run_mut L08 '    promptChars false 12 ≠ promptChars false 34 := by decide' '    promptChars false 12 = promptChars false 34 := by decide' 'the non-vacuity witness is inverted and must stop elaborating'
