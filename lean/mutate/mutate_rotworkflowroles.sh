...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotWorkflowRoles.lean (a partial run is not a pass)
#
...
F="Proofs/RotWorkflowRoles.lean"
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotWorkflowRoles.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotworkflowroles.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotWorkflowRoles ) >/tmp/mut_pre_rotworkflowroles.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotWorkflowRoles)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotworkflowroles.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotWorkflowRoles ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotWorkflowRoles ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotWorkflowRoles mutation suite ==="
...
run_mut W01 "def greenRecently (bound : Nat) (w : Workflow) : Bool := w.youngestGreenHours ≤ bound" "def greenRecently (bound : Nat) (w : Workflow) : Bool := w.newestRunHours ≤ bound" \
  "a_workflow_that_runs_is_not_a_workflow_that_works -- measure the newest RUN instead of the newest SUCCESS and a workflow red for seven days reads as the freshest in the repository"
run_mut W02 "def writesOnlyDocs (w : Workflow) : Bool := w.writes.all (fun p => docsAllowlist.contains p)" "def writesOnlyDocs (w : Workflow) : Bool := true" \
  "a_docs_manager_may_not_write_to_the_proofs -- a scope check that always says yes lets a scheduled job with write access edit the proof tree"
run_mut W03 "  [\"README.md\", \"CHANGELOG.md\", \"STATUS.md\", \"PROMO.md\", \"topics\", \"tags\"]" "  [\"README.md\", \"CHANGELOG.md\", \"STATUS.md\", \"PROMO.md\", \"topics\", \"tags\", \"lean/Proofs\"]" \
  "the_allowlist_refuses_something -- widening the allowlist by one entry is how a scope check dies quietly, so the refusal witness is stated over that exact path"
run_mut W04 "  roleRespected w && greenRecently bound w" "  roleRespected w" \
  "neither_half_alone_is_enough -- drop the freshness conjunct and a well-scoped workflow that has not succeeded in a year is healthy"
run_mut W05 "  required.contains context" "  true" \
  "an_unregistered_gate_is_not_enforced -- a gate is only a gate if the ruleset actually blocks merging on it; always-true reports every invented context as enforced"
run_mut W06 "  | .docsManager => writesOnlyDocs w" "  | .docsManager => true" \
  "the role split itself -- exempt the documentation managers and the two roles stop being different"
