...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotUpgrade.lean (a successful install is not an upgrade)
#
...
#
# WHAT THIS SUITE IS AIMED AT. RotUpgrade states why the round-1 A/B verdict
...
F="Proofs/RotUpgrade.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotUpgrade.olean
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotUpgrade ) >/tmp/mut_pre_rotupgrade.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotUpgrade)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotupgrade.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotUpgrade ) >/dev/null 2>&1' EXIT
...
# 2026-08-07, being killed at that ceiling left a MUTATED RotUpgrade.lean on
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotUpgrade ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotUpgrade\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotUpgrade mutation suite ==="
...
# --- mutants ------------------------------------------------------------------
# Every theorem in RotUpgrade is proved by decide or a two-line simp over
...
run_mut U01 \
  '  | some _ => (r, true)' \
  '  | some _ => ({ entry := some (c.offers, cachePath c.offers) }, true)' \
  'install overwrites an existing entry -- the measured no-op branch is gone'

# U02: a clean install installs nothing -- the empty branch stops working
run_mut U02 \
  '  | none => ({ entry := some (c.offers, cachePath c.offers) }, true)' \
  '  | none => ({ entry := none }, true)' \
  'a clean install installs nothing -- the empty branch stops working'

# U03: uninstall keeps the entry -- the repair sequence no longer repairs
run_mut U03 \
  'def uninstall (_r : Registry) : Registry := { entry := none }' \
  'def uninstall (_r : Registry) : Registry := _r' \
  'uninstall keeps the entry -- the repair sequence no longer repairs'

# U04: the cache path drops the version -- two builds share one directory
run_mut U04 \
  '  "plugins/cache/rot-moe/rot-moe/" ++ v' \
  '  "plugins/cache/rot-moe/rot-moe/"' \
  'the cache path drops the version -- two builds share one directory'

# U05: running reports the install PATH instead of the version
run_mut U05 \
  'def running (r : Registry) : Option Ver := r.entry.map Prod.fst' \
  'def running (r : Registry) : Option Ver := r.entry.map Prod.snd' \
  'running reports the install PATH instead of the version'

# U06: the measured starting state is already current -- no disagreement to find
run_mut U06 \
  'def before : Registry := { entry := some ("0.9.2", cachePath "0.9.2") }' \
  'def before : Registry := { entry := some ("1.0.1", cachePath "1.0.1") }' \
  'the measured starting state is already current -- no disagreement to find'

printf '


restoring baseline artifact ... '
if ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotUpgrade ) >"$LOG/restore.log" 2>&1; then
...
if [ "$filtered" -eq 1 ]; then
  echo "=== RotUpgrade: PARTIAL RUN (MUT_ONLY='$MUT_ONLY') -- $killed killed, $survived survived, $discarded discarded, $skipped SKIPPED ==="
  echo "NOT a suite result. $skipped mutants were never applied and prove nothing."
  [ "$survived" -eq 0 ] && [ "$discarded" -eq 0 ] && exit 3
  exit 1
fi
echo "=== RotUpgrade: $killed killed, $survived survived, $discarded discarded ==="
