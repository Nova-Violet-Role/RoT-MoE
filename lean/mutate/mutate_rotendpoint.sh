...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotEndpoint.lean (a saturated endpoint cannot show a win)
#
...
#
# WHAT THIS SUITE IS AIMED AT. RotEndpoint states why the round-1 A/B verdict
...
F="Proofs/RotEndpoint.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotEndpoint.olean
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotEndpoint ) >/tmp/mut_pre_rotendpoint.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotEndpoint)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotendpoint.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEndpoint ) >/dev/null 2>&1' EXIT
...
# 2026-08-07, being killed at that ceiling left a MUTATED RotEndpoint.lean on
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEndpoint ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotEndpoint\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotEndpoint mutation suite ==="
...
# --- mutants ------------------------------------------------------------------
# Every theorem in RotEndpoint is proved by decide or a two-line simp over
...
run_mut E01 \
  ...
  'def favoursRouted (p : Pair) : Bool := control p < routed p' \
  'the win direction is flipped -- fewer counts stops meaning better'

# E02: the routed win count is computed from the control predicate
run_mut E02 \
  'def winsRouted (ps : List Pair) : Nat := (ps.filter favoursRouted).length' \
  'def winsRouted (ps : List Pair) : Nat := (ps.filter favoursControl).length' \
  'the routed win count is computed from the control predicate'

# E03: the trailing-question control arm is moved off the floor -- saturation vanishes
run_mut E03 \
  'def trailingQuestion : List Pair := List.replicate 88 (0, 0)' \
  'def trailingQuestion : List Pair := List.replicate 88 (0, 1)' \
  'the trailing-question control arm is moved off the floor -- saturation vanishes'

# E04: the narration routed arm gains counts -- the tie row stops being a tie
run_mut E04 \
  'def selfNarration : List Pair := List.replicate 88 (0, 0)' \
  'def selfNarration : List Pair := List.replicate 88 (1, 0)' \
  'the narration routed arm gains counts -- the tie row stops being a tie'

# E05: the hedging result is flipped to favour routing -- the negative finding disappears
run_mut E05 \
  '  List.replicate 83 (0, 0) ++ List.replicate 4 (1, 0) ++ [(0, 1)]' \
  '  List.replicate 83 (0, 0) ++ List.replicate 4 (0, 1) ++ [(0, 1)]' \
  'the hedging result is flipped to favour routing -- the negative finding disappears'

# E06: both predicates now point the same way -- no direction left to disagree about
run_mut E06 \
...
if ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEndpoint ) >"$LOG/restore.log" 2>&1; then
...
if [ "$filtered" -eq 1 ]; then
  echo "=== RotEndpoint: PARTIAL RUN (MUT_ONLY='$MUT_ONLY') -- $killed killed, $survived survived, $discarded discarded, $skipped SKIPPED ==="
  echo "NOT a suite result. $skipped mutants were never applied and prove nothing."
  [ "$survived" -eq 0 ] && [ "$discarded" -eq 0 ] && exit 3
  exit 1
fi
echo "=== RotEndpoint: $killed killed, $survived survived, $discarded discarded ==="
