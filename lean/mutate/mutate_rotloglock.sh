...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotLogLock.lean (a partial run is not a pass)
#
...
# name is a snapshot waiting to drift -- Proofs/RotLogLock.lean,
# a_derived_extractor_always_attributes.
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotLogLock.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotloglock.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotLogLock ) >/tmp/mut_pre_rotloglock.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotLogLock)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotloglock.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLogLock ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotLogLock ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotLogLock mutation suite ==="
...
run_mut L01 "def intruder : Write := ⟨2, 15, 15, 25⟩" "def intruder : Write := ⟨2, 25, 25, 35⟩" \
  "unlocked_admits_a_split -- move the intruder clear of the victim and the module proves nothing about the 265 measured lines: the witness IS the non-vacuity"
run_mut L02 "  (a.finish ≤ b.start) || (b.finish ≤ a.start)" "  true" \
  "the_measured_pair_is_not_excluded / admitting_two_holders_restores_the_defect -- a lock that always CLAIMS exclusion is the CreateDirectory bug in Lean form"
run_mut L03 "def corruptBy (refuse : Bool) : Nat := if refuse then 0 else 2" "def corruptBy (refuse : Bool) : Nat := if refuse then 0 else 0" \
  "refusing_beats_writing_unlocked -- if an unlocked write cost nothing, refusing would be the wrong policy and the router should just write"
run_mut L04 "def lostBy (refuse : Bool) : Nat := if refuse then 1 else 0" "def lostBy (refuse : Bool) : Nat := if refuse then 0 else 0" \
  "a_refusal_is_visible -- a refusal that costs nothing is a SILENT drop, indistinguishable from a router that never fired"
run_mut L05 "  (a.start < b.termAt) && (b.termAt < a.finish)" "  false" \
  "splitsRecord itself: if no schedule can split a record, every theorem here is vacuously true and the measured corruption is unexplained"
...
# downgraded to a skip -- Proofs/RotLogLock.lean, a_survivor_outranks_a_skip.
