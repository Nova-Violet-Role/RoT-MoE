...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotGaugePositivity.lean (a partial run is not a pass)
#
...
F="Proofs/RotGaugePositivity.lean"
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotGaugePositivity.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotgaugepositivity.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotGaugePositivity ) >/tmp/mut_pre_rotgaugepositivity.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotGaugePositivity)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotgaugepositivity.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGaugePositivity ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGaugePositivity ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotGaugePositivity mutation suite ==="
...
run_mut G01 "  if breadth > 0 then act / breadth else 0" "  if breadth > 0 then act / breadth else 1" \
  "the_guard_is_total_at_zero_breadth -- the zero-breadth branch is the production case (a turn on which no lens fires); any other value there is a fabricated share of nothing"
run_mut G02 "def termValue (t : Term) : Nat := t.lam * t.sig * t.mu" "def termValue (t : Term) : Nat := t.lam * t.sig + t.mu" \
  "a_term_vanishes_only_when_a_factor_does -- turn one product into a sum and a lens with mu can no longer be silenced by a zero lambda, which destroys the attribution the whole module exists for"
run_mut G03 "  | t :: ts => termValue t + gaugeSum ts" "  | t :: ts => gaugeSum ts" \
  "the_witness_gauge_is_positive -- drop the head term and the ensemble sums only its tail; every lens would have to be checked twice to notice"
run_mut G04 "  | [] => 0" "  | [] => 1" \
  "the_forbidden_zero_needs_every_lens_dead -- a non-zero empty sum means a gauge over NO lenses is positive, which is exactly the phantom reading the zero-is-a-violation law was written to catch"
run_mut G05 "def K : Nat := 9" "def K : Nat := 8" \
  "the_witness_is_the_full_ensemble -- K is nine on every record the live run wrote; eight is the pre-Claude-lens ensemble and would silently drop a lens from the average"
run_mut G06 "  , { lam := 100, sig := 50, mu :=  95 } ] -- Soleil" "  ] -- Soleil" \
  "the_witness_is_the_full_ensemble -- remove a lens from the witness and the anti-vacuity check is running on eight, so the module would be proving things about an ensemble it does not have"
