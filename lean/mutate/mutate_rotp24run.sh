...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotP24Run.lean (the P2.4 process observables)
#
...
# DISCARDED != SURVIVED.
#
# AIMED AT the measured literals of a run that came back AGAINST the router.
# An unfavourable result is the easiest thing in this repository to soften by
# editing a number, so every measured count is mutated here and every theorem
# that rests on one must die. A suite where the CONTRADICTED verdict survives a
# rewritten literal would mean the verdict was never bound to the measurement.
# =============================================================================
set -uo pipefail
cd "$(dirname "$0")/.."

F="Proofs/RotP24Run.lean"
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotP24Run.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutp24run.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotP24Run ) >/tmp/mut_pre_rotp24run.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotP24Run)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotp24run.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotP24Run ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotP24Run ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
run_mut W01 'def o4Forward : NullControl.Comparison := ⟨40, 0⟩' 'def o4Forward : NullControl.Comparison := ⟨40, 40⟩' 'the forward sweep is flipped to favour the routed arm in every pair -- o4_favours_the_routed_arm_in_no_pair, o4_contradicts_the_claimed_direction and p24_does_not_establish_better_work must all die'
run_mut W02 'def o4Reverse : NullControl.Comparison := ⟨39, 0⟩' 'def o4Reverse : NullControl.Comparison := ⟨39, 20⟩' 'the reverse ordering is softened to a near-tie, so the sweep and the both-orderings claims must fail'
run_mut W03 'def aaControl : NullControl.Comparison := ⟨6, 2⟩' 'def aaControl : NullControl.Comparison := ⟨6, 0⟩' 'the A/A control is made to sweep -- the_null_control_did_not_sweep must die, because a swept control forbids attributing the A/B sweep'
run_mut W04 'def o1Forward : NullControl.Comparison := ⟨0, 0⟩' 'def o1Forward : NullControl.Comparison := ⟨5, 0⟩' 'a saturated observable is given pairs it never had -- three_of_four_observables_are_saturated must die'
run_mut W05 'def toolCalls : (Nat × Nat) × (Nat × Nat) := ((49, 103), (55, 109))' 'def toolCalls : (Nat × Nat) × (Nat × Nat) := ((149, 103), (55, 109))' 'the confound is erased by inflating the tool calls of the routed arm -- the_unrouted_arm_had_the_larger_haystack must die'
run_mut W06 'def evidenceBytes : (Nat × Nat) × (Nat × Nat) := ((9652, 76150), (9864, 46461))' 'def evidenceBytes : (Nat × Nat) × (Nat × Nat) := ((99652, 76150), (9864, 46461))' 'same erasure by bytes rather than calls; the haystack asymmetry must not survive either spelling'
run_mut W07 '    (h : c.discordant = 0) : NullControl.runVerdict c = Verdict.notSupported := by' '    (h : c.discordant = 0) : NullControl.runVerdict c = Verdict.supported := by' 'the saturation lemma is inverted to claim a zero-pair observable SUPPORTS the claim -- it must not be provable'
run_mut W08 '    0 < o4Forward.discordant ∧ 0 < o4Reverse.discordant := by decide' '    0 < o4Forward.discordant ∧ 0 = o4Reverse.discordant := by decide' 'the reverse ordering is asserted to have produced no pairs, contradicting the measurement it is bound to'
