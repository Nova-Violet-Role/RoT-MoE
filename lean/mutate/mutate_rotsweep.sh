...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotSweep.lean (a partial run is not a pass)
#
...
# name is a snapshot waiting to drift -- Proofs/RotSweep.lean,
# a_derived_extractor_always_attributes.
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotSweep.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotsweep.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotSweep ) >/tmp/mut_pre_rotsweep.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotSweep)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotsweep.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotSweep ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotSweep ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
run_mut S01 "  | true,  false, true => 1" "  | true,  false, true => 0" \
  "truncation_is_refused / a_verdict_requires_every_declared_mutant -- a run that stopped short stops being refused, which is exactly the prefix-verdict defect"
run_mut S02 "  | true,  true, _     => 1" "  | true,  true, _     => 0" \
  "a_clean_verdict_has_no_survivor_and_no_discard -- a SURVIVOR would be scored a pass; this is the shape every suite guard exists to stop"
run_mut S03 "def reportedAt (r : Run) (k : Nat) : Nat := min r.killed k" "def reportedAt (r : Run) (k : Nat) : Nat := r.killed" \
  "early_summary_under_reports / early_echo_is_indistinguishable_from_truncation -- if the summary were always exact, the rotlog defect could not exist and the module would model nothing"
run_mut S04 "def sweep : Run := ⟨634, 634, 634, 0, 0⟩" "def sweep : Run := ⟨634, 633, 633, 0, 0⟩" \
  "the_sweep_is_clean / the_sweep_ran_everything -- the MEASURED sweep must be a complete run, not 633 of 634"
run_mut S05 "  | false, _,    _     => 2" "  | false, _,    _     => 0" \
  "the incoherent-counter guard: killed+survived+discarded must equal ran, or the observation is not about this run"
...
# downgraded to a skip -- Proofs/RotSweep.lean, a_survivor_outranks_a_skip.
