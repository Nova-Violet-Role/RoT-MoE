...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotPartialRun.lean (a partial run is not a pass)
#
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotPartialRun.olean
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotPartialRun ) >/tmp/mut_pre_rotpartialrun.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotPartialRun)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotpartialrun.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotPartialRun ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotPartialRun ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotPartialRun mutation suite ==="
...
run_mut P01 'def honestVerdict (r : Run) : Bool := r.failed == 0 && notrun r == 0' 'def honestVerdict (r : Run) : Bool := r.failed == 0' 'THE SHIPPED BUG RESTORED: the verdict consults only the failure count again, so a run of zero steps passes'
run_mut P02 'def notrun (r : Run) : Nat := r.total - r.ran' 'def notrun (r : Run) : Nat := r.ran - r.total' 'the subtraction is reversed, so a run that executed nothing reports nothing outstanding'
run_mut P03 'def theIncident : Run := ⟨75, 0, 0⟩' 'def theIncident : Run := ⟨75, 75, 0⟩' 'the measured incident is rewritten as a full sweep, erasing the case the fix exists for'
run_mut P04 'def fullSweep : Run := ⟨75, 75, 0⟩' 'def fullSweep : Run := ⟨75, 74, 0⟩' 'the full sweep is one step short, so the honest verdict can no longer accept any run'
run_mut P05 'def measuredSteps : Nat := 75' 'def measuredSteps : Nat := 74' 'the extracted step count no longer matches the two complementary windows that covered it'
run_mut P06 'def measuredWindow2 : Nat := 35' 'def measuredWindow2 : Nat := 34' 'the second window shrinks, leaving a one-step gap the coverage guard must catch'
