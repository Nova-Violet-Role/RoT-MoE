...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotVerdictDecision.lean (a partial run is not a pass)
#
...
F="Proofs/RotVerdictDecision.lean"
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotVerdictDecision.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotverdictdecision.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotVerdictDecision ) >/tmp/mut_pre_rotverdictdecision.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotVerdictDecision)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotverdictdecision.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotVerdictDecision ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotVerdictDecision ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotVerdictDecision mutation suite ==="
...
run_mut V01 "  | _, [] => .unmeasured" "  | _, [] => .unchanged" \
  "an_absent_verdict_is_never_called_unchanged -- collapsing the third outcome back into the second is precisely the defect: a verdict nobody produced reported as a verdict that did not move"
run_mut V02 "  | old, (a :: as) => if old == (a :: as) then .unchanged else .changed" "  | old, (a :: as) => if old == (a :: as) then .changed else .unchanged" \
  "the_repair_agrees_wherever_the_old_check_was_meaningful -- invert the comparison and the weekly job commits when nothing moved and stays silent when the verdict changed"
run_mut V03 "def changed (old new : Verdict) : Bool := old != new" "def changed (old new : Verdict) : Bool := old == new" \
  "a_vanished_verdict_looks_exactly_like_an_unchanged_one -- the model of the SHIPPED step must keep reporting false on identical input, or the theorem stops describing the workflow it was written about"
run_mut V04 "  | .unmeasured => false" "  | .unmeasured => true" \
  "skipping_and_failing_never_both_apply -- let an unmeasured verdict skip quietly and the third outcome buys nothing; the job is green again with nothing measured"
run_mut V05 "  | .unmeasured => true" "  | .unmeasured => false" \
  "the_silent_generator_now_fails_the_job -- drop the obligation to fail and the alarm exists but never rings"
run_mut V06 "  | .unchanged => true" "  | .unchanged => false" \
  "the_run_that_prompted_this_was_actually_fine -- a real unchanged verdict must still be allowed to skip, or the repair blocks every correct weekly run"
