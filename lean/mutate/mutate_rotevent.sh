...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotEvent.lean (eleven events, and a log that can name which one fired)
#
...
F="Proofs/RotEvent.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotEvent.olean
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotEvent ) >/tmp/mut_pre_rotguard.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotEvent)."
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEvent ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEvent ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotEvent\.lean:[0-9]+" "$LOG/$id.log" \
...
# E01: the sanitiser stops refusing -- every payload is written verbatim into JSON
run_mut E01 \
  '  if !s.toList.isEmpty && s.toList.all isLetter then s else "-"' \
  '  s' \
  'the sanitiser becomes the identity -- a hostile event name reaches the JSON record'

# E02: the emptiness conjunct is dropped -- an empty name is recorded as an event
run_mut E02 \
  '  if !s.toList.isEmpty && s.toList.all isLetter then s else "-"' \
  '  if s.toList.all isLetter then s else "-"' \
  'the empty name is no longer refused -- a record claims an event named ""'

# E03: the letter test is inverted -- only NON-letter names survive
run_mut E03 \
  '  if !s.toList.isEmpty && s.toList.all isLetter then s else "-"' \
  '  if !s.toList.isEmpty && !s.toList.all isLetter then s else "-"' \
  'the guard is inverted -- exactly the hostile names are the ones let through'

# E04: digits are admitted as letters -- the charset widens past what the shell allows
run_mut E04 \
  "  ('A' ≤ c && c ≤ 'Z') || ('a' ≤ c && c ≤ 'z')" \
  "  ('A' ≤ c && c ≤ 'Z') || (' ' ≤ c && c ≤ 'z')" \
  'the charset lower bound slips to space -- the double quote becomes a legal event name'

# E05: an event is dropped from the declared list -- coverage silently shrinks to ten
run_mut E05 \
  '   "PreCompact", "PostCompact"]' \
  '   "PreCompact"]' \
  'the declared list loses PostCompact -- the eleven-event claim is false'
...
run_mut E07 \
  ...
  'the pre-fix binding is inflated -- the eight-event blind spot shrinks to six'

# E08: the non-vacuity witness is weakened to the refusal value itself
run_mut E08 \
  '  if !s.toList.isEmpty && s.toList.all isLetter then s else "-"' \
  '  "-"' \
  'the sanitiser destroys every name -- the log can no longer identify ANY event'

printf '


restoring baseline artifact ... '
if ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotEvent ) >"$LOG/restore.log" 2>&1; then
...
if [ "$filtered" -eq 1 ]; then
  echo "=== RotEvent: PARTIAL RUN (MUT_ONLY='$MUT_ONLY') -- $killed killed, $survived survived, $discarded discarded, $skipped SKIPPED ==="
  echo "NOT a suite result. $skipped mutants were never applied and prove nothing."
  [ "$survived" -eq 0 ] && [ "$discarded" -eq 0 ] && exit 3
  exit 1
fi
echo "=== RotEvent: $killed killed, $survived survived, $discarded discarded ==="
