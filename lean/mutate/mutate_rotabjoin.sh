...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotAbJoin.lean (a session id that reaches a filename)
#
...
F="Proofs/RotAbJoin.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotAbJoin.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutabjoin.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotAbJoin ) >/tmp/mut_pre_rotabjoin.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotAbJoin)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotabjoin.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotAbJoin ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotAbJoin ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotAbJoin\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotAbJoin mutation suite ==="

# WHAT THIS SUITE IS AIMED AT.
#
# The module says the A/B's disarm check measures the wrong thing: a GLOBAL
# record delta over a shared, rotating log, where the question is about ONE run.
# It is unsound in both directions -- foreign traffic condemns an innocent arm,
# and rotation absolves a guilty one.
#
# A04 and A05 are the ones to watch. Both attack the ROTATION half of the
# argument, which is the half that produces a FALSE PASS. If they survive, the
# module has only proved the harmless failure mode (a noisy red) and says
# nothing about the dangerous one (a quiet green).

run_mut A01 \
  "  (recs.filter (fun r => r.session == s && r.isRoute)).length" \
  "  (recs.filter (fun r => r.isRoute)).length" \
  "join_ignores_foreign_traffic -- foreign sessions would be counted"

run_mut A02 \
  "  (recs.filter (fun r => r.session == s && r.isRoute)).length" \
  "  (recs.filter (fun r => r.session == s)).length" \
  "join_ignores_non_route_records -- a gauge line would read as routing"

run_mut A03 \
  "  sessionRouteCount recs s == 0" \
  "  sessionRouteCount recs s != 0" \
  "join_never_false_passes -- the verdict inverted"

run_mut A04 \
  "  (before + written - dropped : Int) - before" \
  "  (before + written : Int) - before" \
  "delta_false_passes_under_rotation -- drop rotation and the false pass vanishes"

run_mut A05 \
  "#guard disarmedByDelta 100 3 3 = true" \
  "#guard disarmedByDelta 100 3 3 = false" \
  "the rotation guard -- if this survives the false pass is not pinned"

run_mut A06 \
  "  globalDelta before written dropped == 0" \
  "  true" \
  "delta_false_alarms_on_foreign_traffic -- the delta would always pass"

run_mut A07 \
  "def mixed : List Rec := [⟨\"other-1\", true⟩, ⟨\"armB\", true⟩, ⟨\"armB\", false⟩]" \
  "def mixed : List Rec := [⟨\"other-1\", true⟩, ⟨\"armB\", false⟩, ⟨\"armB\", false⟩]" \
  "the mixed-corpus guards -- armB's route record removed"

run_mut A08 \
  "#guard sessionRouteCount foreignOnly \"armB\" = 0" \
  "#guard sessionRouteCount foreignOnly \"armB\" = 2" \
  "the foreign-traffic guard itself"
...
# Measured 2026-08-09 on Proofs.RotAbJoin.
cp "$BAK" "$F" 2>/dev/null
( cd ${LEAN_ROOT:-.} && lake build Proofs.RotAbJoin ) >/dev/null 2>&1
_base=$?
if [ "$_base" -ne 0 ]; then
