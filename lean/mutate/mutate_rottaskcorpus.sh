...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotTaskCorpus.lean (a partial run is not a pass)
#
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotTaskCorpus.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrottaskcorpus.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotTaskCorpus ) >/tmp/mut_pre_rottaskcorpus.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotTaskCorpus)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rottaskcorpus.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotTaskCorpus ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotTaskCorpus ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotTaskCorpus mutation suite ==="
...
run_mut T01 "def families : Nat := 4" "def families : Nat := 3" \
  "the_plan_holds_forty_tasks, every_family_has_ten -- drop a seed family and the corpus is 30 tasks while every per-family theorem still reads as if it covered the design; section 4 of the preregistration fixes four families before either arm runs"
run_mut T02 "def lanes : Nat := 10" "def lanes : Nat := 9" \
  "the_plan_holds_forty_tasks, every_lane_has_four, the_plan_covers_every_lane -- THE mutant for this module: nine lanes is the exact off-by-one the existing bench key shipped with, leaving CONVERGENT (the fallback every unmatched prompt takes) unexercised"
run_mut T03 "def margin (t : Task) (w : World) : Nat := carefulScore t w - naiveScore t w" "def margin (t : Task) (w : World) : Nat := carefulScore t w" \
  "an_indiscriminate_task_cannot_separate_the_arms, the_margin_is_exactly_discrimination -- a margin that ignores the naive arm scores every task as a win, so a corpus of ties would report a clean sweep"
run_mut T04 "def naiveScore (t : Task) (w : World) : Nat := if t.naiveAt w = t.truthAt w then 1 else 0" "def naiveScore (t : Task) (w : World) : Nat := 0" \
  "an_indiscriminate_task_cannot_separate_the_arms, a_corpus_of_ties_is_vacuous -- assume the naive arm is never right and the whole discrimination requirement evaporates; this is the assumption the corpus exists to test rather than make"
run_mut T05 "def discriminates (t : Task) (w : World) : Bool := t.naiveAt w != t.truthAt w" "def discriminates (t : Task) (w : World) : Bool := true" \
  "sample_stops_discriminating_at_world_three, the_frozen_check_claims_discrimination_that_is_not_there -- declare every task discriminating and checker/corpus-verify.sh's central assertion becomes unfalsifiable"
run_mut T06 "  naiveAt := fun w => if w = 3 then 38 else 37 + w" "  naiveAt := fun w => 37 + w" \
  "sample_stops_discriminating_at_world_three, the_frozen_check_claims_discrimination_that_is_not_there -- remove the world where the two instruments agree and the frozen-value hazard has no witness, which would make the case for storing commands instead of numbers unprovable"
run_mut T07 "  t.naiveAt w != frozen" "  discriminates t w" \
  "the_frozen_check_claims_discrimination_that_is_not_there, the_frozen_check_also_denies_real_discrimination -- define the frozen check AS the derived one and the difference between a snapshot and a re-measurement disappears by fiat"
run_mut T08 "(List.range lanes).map (fun l => (f, l))" "(List.range lanes).map (fun l => (f, 0))" \
  "the_plan_repeats_nothing, every_lane_has_four, the_plan_covers_every_lane -- collapse every task onto one lane: the corpus still has 40 rows, which is exactly why forty_rows_does_not_imply_coverage had to be stated"
