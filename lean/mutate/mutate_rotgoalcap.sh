...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotGoalCap.lean (a partial run is not a pass)
#
...
# name is a snapshot waiting to drift -- Proofs/RotGoalCap.lean,
# a_derived_extractor_always_attributes.
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotGoalCap.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotgoalcap.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotGoalCap ) >/tmp/mut_pre_rotgoalcap.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotGoalCap)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotgoalcap.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGoalCap ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGoalCap ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotGoalCap mutation suite ==="
...
run_mut G01 "  (xs.filter (fun i => i.executed)).map (·.cap)" "  xs.map (·.cap)" \
  "patching_the_unexecuted_is_invisible -- if effective ignores the executed flag, a patch to a dormant image becomes visible and the whole regression cannot be stated"
run_mut G02 "decide (len ≤ i.cap)" "decide (len < i.cap)" \
  "a_gap_admits_a_split -- off-by-one at the boundary: cap+1 is refused by BOTH, so the splitting witness stops existing"
run_mut G03 "#guard effective estateBefore = [4000, 4000]" "#guard effective estateBefore = [10000, 10000]" \
  "the measured incident -- the EXECUTED images were the stale 4000 ones, not the raised 10000 ones"
run_mut G04 "def measuredGoalLength : Nat := 4040" "def measuredGoalLength : Nat := 3999" \
  "agree estateBefore = false -- below 4000 every image accepts, so the estate stops disagreeing and the incident vanishes"
run_mut G05 "measuredGoalLength < 9999" "measuredGoalLength < 4000" \
  "the headroom example -- 4040 does NOT fit under 4000, which is the entire refusal"
...
# downgraded to a skip -- Proofs/RotGoalCap.lean, a_survivor_outranks_a_skip.
