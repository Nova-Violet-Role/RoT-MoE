...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotReadmeTable.lean (a partial run is not a pass)
#
...
MOD=${F##*/}; MOD=${MOD%.lean}
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotReadmeTable.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutrotreadmetable.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotReadmeTable ) >/tmp/mut_pre_rotreadmetable.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotReadmeTable)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotreadmetable.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotReadmeTable ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotReadmeTable ) > "$LOG/$id.log" 2>&1
  local ec=$?
...
echo "=== RotReadmeTable mutation suite ==="
...
run_mut R01 "def covers (c : Catalogue) (t : Table) : Bool := c.all (fun p => documented t p.1)" "def covers (c : Catalogue) (t : Table) : Bool := c.any (fun p => documented t p.1)" \
  "naming_the_gaps_agrees_with_the_coverage_test -- coverage means EVERY module is documented; an any-quantifier passes a README that mentions one file and forgets sixty-seven"
run_mut R02 "def exact (c : Catalogue) (t : Table) : Bool := t.all (exactRow c)" "def exact (c : Catalogue) (t : Table) : Bool := t.any (exactRow c)" \
  "two_mentions_that_disagree_are_still_caught / coverage_without_the_recount_admits_an_inflated_claim -- one correct row would excuse every wrong one"
run_mut R03 "  c.any (fun p => p.1 == r.1 && p.2 == r.2)" "  c.any (fun p => p.1 == r.1)" \
  "two_mentions_that_disagree_are_still_caught -- match the module name but stop comparing the COUNT and the recount stops recounting"
run_mut R04 "  (c.filter (fun p => !documented t p.1)).map Prod.fst" "  (c.filter (fun p => documented t p.1)).map Prod.fst" \
  "coverage_by_name_catches_both_and_names_them / naming_the_gaps_agrees_with_the_coverage_test -- drop the negation and the diagnostic names the modules that ARE documented"
run_mut R05 "  t.foldl (fun acc r => if acc.any (fun q => q.1 == r.1) then acc else acc ++ [r]) []" "  t.foldl (fun acc r => acc ++ [r]) []" \
  "the_distinct_sum_accepts_it_and_still_rejects_an_omission -- without deduplication the appendix double-counts every narrated module and a correct README goes red"
run_mut R06 "def documented (t : Table) (name : String) : Bool := t.any (fun r => r.1 == name)" "def documented (t : Table) (name : String) : Bool := true" \
  "every coverage theorem -- a documented-check that always says yes reports full coverage of a README with no rows at all"
