...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotCorpus.lean (an untagged corpus attributes to whoever is asking)
#
...
#
# WHAT THIS SUITE IS AIMED AT. RotCorpus states why the round-1 A/B verdict
...
F="Proofs/RotCorpus.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotCorpus.olean
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotCorpus ) >/tmp/mut_pre_rotcorpus.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotCorpus)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotcorpus.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotCorpus ) >/dev/null 2>&1' EXIT
...
# 2026-08-07, being killed at that ceiling left a MUTATED RotCorpus.lean on
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotCorpus ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotCorpus\.lean:[0-9]+" "$LOG/$id.log" \
...
# --- mutants ------------------------------------------------------------------
# Every theorem in RotCorpus is proved by decide or a two-line simp over
...
run_mut P01 \
  ...
  '  | Rec.route x :: rest => (if x = l then 1 else 1) + cnt l rest' \
  'cnt counts every firing regardless of lane -- no lane can lead another'

# P02: a marker no longer takes effect -- attribution can never start
run_mut P02 \
  '  | Rec.mark v :: rest, _ => assign rest (some v)' \
  '  | Rec.mark v :: rest, c => assign rest c' \
  'a marker no longer takes effect -- attribution can never start'

# P03: every firing is recorded unattributed even after a marker
run_mut P03 \
  '  | Rec.route l :: rest, cur => (cur, l) :: assign rest cur' \
  '  | Rec.route l :: rest, cur => (none, l) :: assign rest cur' \
  'every firing is recorded unattributed even after a marker'

# P04: the version in effect never advances -- appended runs inherit the wrong build
run_mut P04 \
  '  | Rec.mark v :: rest, _ => finalVer rest (some v)' \
  '  | Rec.mark v :: rest, c => finalVer rest c' \
  'the version in effect never advances -- appended runs inherit the wrong build'

# P05: both runs lead on the same lane -- pooling can no longer invent a third
run_mut P05 \
  ...
  'both runs lead on the same lane -- pooling can no longer invent a third'

# P06: onlyVer returns the firings of every OTHER version -- the partition is inverted
run_mut P06 \
  '  (xs.filter (fun p => p.1 = some v)).map Prod.snd' \
  '  (xs.filter (fun p => p.1 != some v)).map Prod.snd' \
  'onlyVer returns the firings of every OTHER version -- the partition is inverted'

printf '


restoring baseline artifact ... '
if ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotCorpus ) >"$LOG/restore.log" 2>&1; then
...
if [ "$filtered" -eq 1 ]; then
  echo "=== RotCorpus: PARTIAL RUN (MUT_ONLY='$MUT_ONLY') -- $killed killed, $survived survived, $discarded discarded, $skipped SKIPPED ==="
  echo "NOT a suite result. $skipped mutants were never applied and prove nothing."
  [ "$survived" -eq 0 ] && [ "$discarded" -eq 0 ] && exit 3
  exit 1
fi
echo "=== RotCorpus: $killed killed, $survived survived, $discarded discarded ==="
