...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotAbVerdict.lean (a session id that reaches a filename)
#
...
F="Proofs/RotAbVerdict.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotAbVerdict.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutabverdict.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotAbVerdict ) >/tmp/mut_pre_rotabverdict.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotAbVerdict)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotabverdict.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotAbVerdict ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotAbVerdict ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotAbVerdict\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotAbVerdict mutation suite ==="

# WHAT THIS SUITE IS AIMED AT.
#
# The module fixes what the A/B is allowed to claim: the headline (29 wins,
# p=1.09e-5) is real, and 27 of those wins are explained by the routed answer
# being shorter, so the advantage does not survive deconfounding.
#
# V01 and V02 are the ones that matter. Both try to make the verdict pass. If
# either survives, this file no longer prevents the headline from being quoted
# as if it were the result -- which is the single thing it exists to prevent.

run_mut V01 \
  "def survives (unexplained losses : Nat) : Bool := losses < unexplained" \
  "def survives (unexplained losses : Nat) : Bool := losses <= unexplained" \
  "ties_do_not_survive -- a tie would count as an advantage"

run_mut V02 \
  "def survives (unexplained losses : Nat) : Bool := losses < unexplained" \
  "def survives (unexplained losses : Nat) : Bool := true" \
  "measured_does_not_survive -- the verdict would always pass"

run_mut V03 \
  "def measuredUnexplained : Nat := 2" \
  "def measuredUnexplained : Nat := 5" \
  "the measured deconfounded count -- with 5 the corpus would 'survive'"

run_mut V04 \
  "def measuredLosses      : Nat := 4" \
  "def measuredLosses      : Nat := 1" \
  "the measured losses -- fewer losses would flip the verdict"

run_mut V05 \
  "def routedWin  (t : Turn) : Bool := !t.routedViolates && t.unroutedViolates" \
  "def routedWin  (t : Turn) : Bool := !t.routedViolates || t.unroutedViolates" \
  "win_and_loss_exclusive -- a turn could be both a win and a loss"

run_mut V06 \
  "def explainedByBrevity (t : Turn) : Bool := routedWin t && t.routedLen < t.unroutedLen" \
  "def explainedByBrevity (t : Turn) : Bool := routedWin t && t.unroutedLen < t.routedLen" \
  "the brevity confound, inverted -- wins would be credited the wrong way"

run_mut V07 \
  "def unexplainedWin (t : Turn) : Bool := routedWin t && t.unroutedLen ≤ t.routedLen" \
  "def unexplainedWin (t : Turn) : Bool := routedWin t" \
  "win_partition_disjoint -- every win would count as evidence"

run_mut V08 \
  "#guard 27 + measuredUnexplained = measuredHeadlineWins" \
  "#guard 26 + measuredUnexplained = measuredHeadlineWins" \
  "the split of the headline into explained and unexplained"
...
# Measured 2026-08-09 on Proofs.RotAbVerdict.
cp "$BAK" "$F" 2>/dev/null
( cd ${LEAN_ROOT:-.} && lake build Proofs.RotAbVerdict ) >/dev/null 2>&1
_base=$?
if [ "$_base" -ne 0 ]; then
