...
#
# =============================================================================
# MUTATION SUITE -- Proofs/RotGrounding.lean (a session id that reaches a filename)
#
...
F="Proofs/RotGrounding.lean"
BAK="$F.mutbak"
OLEAN=${LEAN_ROOT:-.}/.lake/build/lib/lean/Proofs/RotGrounding.olean
LOG="$(mktemp -d "${TMPDIR:-/tmp}/mutgrounding.XXXXXX")"
...
if ! ( cd "${LEAN_ROOT:-.}" && lake build Proofs.RotGrounding ) >/tmp/mut_pre_rotgrounding.log 2>&1; then
  echo "FATAL: the UNMUTATED baseline does not build (Proofs.RotGrounding)."
  echo "A kill measured against a red baseline is unattributable. Fix the tree first."
  tail -5 /tmp/mut_pre_rotgrounding.log
  exit 2
fi
...
trap 'cp "$BAK" "$F" 2>/dev/null; rm -f "$BAK"; ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGrounding ) >/dev/null 2>&1' EXIT
...
run_mut() {
  ...
  rm -f "$OLEAN"
  ( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGrounding ) > "$LOG/$id.log" 2>&1
  local ec=$?
  ...
  if [ "$ec" -eq 0 ]; then
    ...
    local dead
    dead=$(grep -oE "^error: Proofs/RotGrounding\.lean:[0-9]+" "$LOG/$id.log" \
...
echo "=== RotGrounding mutation suite ==="

# WHAT THIS SUITE IS AIMED AT.
#
# The module records that the grounding metric's 8-0 advantage vanished once
# CLAIM VOLUME was held constant (18 matched pairs, all tied). G01/G05 attack
# the volume-matching itself -- if either survives, an arm that simply asserts
# less could be credited with being more accurate, which is the whole error
# this module exists to prevent.

run_mut G01 \
  "def volumeMatched (p : Pair) : Bool := p.routedCited == p.unroutedCited" \
  "def volumeMatched (p : Pair) : Bool := true" \
  "unmatched_pairs_are_not_evidence -- unequal claim counts would be comparable"

run_mut G02 \
  "def establishesAdvantage (better worse : Nat) : Bool := 0 < better && worse < better" \
  "def establishesAdvantage (better worse : Nat) : Bool := 0 < better && worse <= better" \
  "ties_establish_nothing -- a tie would count as an advantage"

run_mut G03 \
  "def vmRoutedBetter : Nat := 0" \
  "def vmRoutedBetter : Nat := 5" \
  "grounding_establishes_no_advantage -- the measured null becomes a win"

run_mut G04 \
  "def vmPairs : Nat := 18" \
  "def vmPairs : Nat := 19" \
  "all_matched_pairs_tied -- the measured pair count"

run_mut G05 \
  "def routedBetter (p : Pair) : Bool := volumeMatched p && p.unroutedValid < p.routedValid" \
  "def routedBetter (p : Pair) : Bool := volumeMatched p || p.unroutedValid < p.routedValid" \
  "unmatched_pairs_are_not_evidence -- matching would stop being required"

run_mut G06 \
  "#guard routedBetter ⟨1, 1, 5, 4⟩ = false" \
  "#guard routedBetter ⟨1, 1, 5, 4⟩ = true" \
  "the selectivity trap guard: cite less, look perfect, still not a win"

run_mut G07 \
  "#guard 178 - 149 = 29" \
  "#guard 178 - 149 = 28" \
  "the measured absolute false-statement count"

run_mut G08 \
  "theorem advantage_is_reachable : establishesAdvantage 7 2 = true := by decide" \
  "theorem advantage_is_reachable : establishesAdvantage 7 2 = false := by decide" \
  "the non-vacuity witness -- the rule must be able to PASS"
...
# Measured 2026-08-09 on Proofs.RotGrounding.
cp "$BAK" "$F" 2>/dev/null
( cd ${LEAN_ROOT:-.} && lake build Proofs.RotGrounding ) >/dev/null 2>&1
_base=$?
if [ "$_base" -ne 0 ]; then
