...
def shipped : List Gate :=
  ...
  , f "mutation discipline"
    ...
    -- still EXIST. FAST because it is static -- no build, no mutation -- and
  ...
  , f "hook timeout"
  -- FAST, and it must be: it re-derives the A/B figures the CHANGELOG publishes
  -- from the committed per-turn corpus. Those numbers are a CLAIM about what
  -- the router does, and a claim in the prose that no gate re-computes is the
  -- same defect as an uncounted theorem. A trigger path would be dead config
  ...
  -- A timeout, an OOM and an unreadable olean are all "I could not check".
  -- Only a kernel refusal is "the check failed". The gate that keeps those
  -- two apart is fast because it runs on a fixture, never on a real build.
  , f "kernel verdict class (a non-answer is not a rejection)"
  ...
  -- Binds Proofs/RotInject.lean to the hooks that actually run: the accepting
  -- set is parsed OUT of the Lean source and compared to the arrays the shell
