...
first version of this generator reported a single figure of 82 gates against a
runner that reported 72 -- and the live figures are in the digest above, not in
this sentence. That is deliberate. When this gate count next moves, the
generated table moves with it and this paragraph does not; a hand-written number
here would be a second claim about the same fact, with no generator behind it
and nothing to keep the two in step.
...
## How old is the FULL-only tier?

The table below separates the ten FULL-only gates from the default table, which
answers *which* gates a bare sweep skips. It does not answer *when those gates
last ran*, and until recently nothing did. No commit triggers them, no push
triggers them, and the instruction to run them before publishing lived in prose
-- a bound with no assertor, which this repository has repeatedly found to be
the same thing as no bound at all.

`checker/full-freshness.sh` (R22) is that assertor. A full run that reaches the
end with zero red gates stamps a receipt naming the commit it ran on; the gate
reads that receipt and refuses when it is missing, unparseable, names a commit
this repository has never seen, names a commit that is not an ancestor of HEAD,
or sits more than `FULL_MAX_DRIFT` commits behind.

Its triggers are the act of releasing -- `checker/release`, `RELEASE.md`,
`.github/workflows/release` -- and the first list was wider and wrong. It named
the publish *surface*, including `.claude-plugin/` and `CITATION.cff`, which
`checker/repo-complete.sh --write` regenerates whenever a theorem count moves.
That list fired on nearly every substantive commit against a gate that is red
until a full run exists, which does not produce a stricter repository; it
produces a hook people switch off. It also self-triggered. A gate whose verdict
depends on its own source should self-trigger, and one whose verdict depends on
external state should not: editing this script cannot change whether the full
tier is stale, so the self-trigger bought no coverage while deadlocking the
gate's own repair. Its four internal controls guard the script itself.

The receipt cannot be written by hand: `--stamp` refuses without
`FULL_RUN_COMPLETED=1`, which only the tail of a green full run sets, and
refuses again if the working tree is dirty -- a run against uncommitted changes
tested a state no commit records, so no sha honestly describes it.

The model is proved rather than argued, in `lean/Proofs/RotFreshness.lean`:

- `a_stale_receipt_can_describe_a_tree_that_no_longer_exists` -- an honestly
  computed verdict, correct when made, disagreeing with the truth now. No
  instrument misbehaved; this is what age alone does.
- `staleness_is_invisible_in_the_verdict` -- two receipts carrying the same
  verdict, one fresh and one stale. The verdict field holds no information
  about its own age, so reading results more carefully cannot recover it.
- `absence_and_green_must_not_agree` -- with no receipt at all the checker
  answers red, so a tier that never ran is distinguishable from one that passed.
- `check_sound` -- every green this design emits came from a fresh receipt.
- `ignoring_freshness_is_unsound` -- the obvious simpler checker, the one that
  believes the receipt, emits green where the present truth is red.

At the time of writing this gate is RED, and the red is the finding: branch
`9.0.0` has never had a full run. A green here would have been the defect.
...
| Measure | Value |
|---|---|
| Gates in the DEFAULT table (a bare sweep runs these) | 73 |
| Of those, fast tier | 48 |
| Of those, deep tier | 25 |
| Gates reachable ONLY under --full | 10 |
| Checker scripts on disk | 84 |
| Scripts no registry row runs | 3 |
| Registry rows naming a missing script | 0 |
| Scripts with control text in CODE | 78 |
...
| 73 | FULL-only tier freshness -- is the newest word on it about THIS tree | deep | checker/release,RELEASE.md | `checker/full-freshness.sh` | CODE |
